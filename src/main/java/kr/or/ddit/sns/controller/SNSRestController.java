...
public class SNSRestController {
    ...
    public ResponseEntity<Map<String, Object>> wordCloud(@RequestBody WordVO vo) {
        ...
        String naver = naverBlog.naverBlogcrawling(vo.getKeyword());
        JsonParser jsonParser = new JsonParser();
        JsonObject jsonObject = (JsonObject) jsonParser.parse(naver);
        JsonArray array = jsonObject.get("items").getAsJsonArray();
        String resultTxt = "";
        List<Map<String, Object>> list = new ArrayList<>();
        for(int i = 0; i<array.size(); i++){
            resultTxt += array.get(i).getAsJsonObject().get("title");
            resultTxt += array.get(i).getAsJsonObject().get("description");
        }
